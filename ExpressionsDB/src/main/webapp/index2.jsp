	...
	function setExpressionData(response) {
		if (response != null) {
			for (data in response) {
				var str = "<tr><td>"
						+ response[data].id
						+ "</td><td><pre><textarea rows='3' cols='30' disabled>"
						+ response[data].namespace
						+ "."
						+ response[data].referenceName
						+ "</textarea></pre></td><td><pre><textarea rows = '3' cols = '20' disabled>"
						...
						+ "</textarea></pre></td><td><pre><textarea rows = '3' cols = '20' disabled>"
						+ response[data].createdTime + "</textarea></pre></td>";
				...
				for (var i = 0; i < listActions.length; i++) {
					str += "<option value='"+listActions[i]+"'>"
							+ listActions[i] + "</option>";
				}
...
<body>
	...
	<table align="center" style="margin-left: 50px; margin-top: 50px"
		cellpadding="10px" border="1px" id="exprTable">
		<tr>
			<td><b>S.No.</b></td>
			<td><b>NAME</b></td>
